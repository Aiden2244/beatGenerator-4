...
class Beat:
    def __init__(
        ...
        self.prog = self.select_progression(
            length=prog_len,
        ...
        self.write_arpeggio_and_moving_chords(
            pattern=arp_pattern,
            note_duration=arp_duration,
            num_phrases=self.num_phrases,
        )
        self.write_melody(num_phrases=self.num_phrases)
        self.drum_mode = self.write_drums(
            drum_mode=drum_mode, num_phrases=self.num_phrases
        )
    ...
    def select_progression(
        self, length=4, force_unique=True, force_root=True, omit_diminished=True
    ):
        """
    ...
    def write_arpeggio_and_moving_chords(
        self, num_phrases=1, note_duration=-1, start_time=0, pattern=-1
    ):
        note_dur = note_duration
        if note_duration == -1:
                    ...
                    if generation_mode == 0:
                        if phrase_duration % 1 == 0:
                            self.midi.addNote(
                                ...
                                pitch=hat1,
                                time=(
                                    start_time + float(phrase_duration / 4) + (4 * i)
                                ),
                                duration=0.25,
                                volume=100,
                            )
        ...
        if random_instruments:
            for i in range(len(INSTRUMENTS)):
                ...
                self.midi.addProgramChange(
                    tracknum=i, channel=i, time=0, program=program
                )
    ...
    def write_progression(
        self, chord_length=4, starting_beat=0, chord_spacing=4, num_phrases=1
    ):
